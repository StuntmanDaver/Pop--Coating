...
- QR payload is the authenticated scanner URL containing the opaque token: `https://app.popsindustrial.com/scan?packet=<packet_token>` in Wave 1, generalized to `https://app.<tenant-domain>/scan?packet=<packet_token>` in Wave 4. Never encode raw job-number URLs such as `/job/JOB-2026-00124`.
...
BEGIN
  ...
  IF v_emp.tenant_id != app.tenant_id() THEN
...
**Scan flow:**
1. Camera scanner reads QR → scanner URL parsed → `packet_token` decoded from the `packet` query param
...
- Test 3: `app.validate_employee_pin(p_employee_id, p_pin)` enforces row lock (concurrent attempts handled atomically with `FOR UPDATE`)
